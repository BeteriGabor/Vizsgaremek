...
const Aviator = () => {
    ...
    useEffect(() => {
        ...
        if (isFlying) {
            ...
            const randomFlightDuration = Math.floor(Math.random() * (12000 - 0 + 1)) + 0;
    ...
    const cashOut = () => {
        if (isFlying) {
            ...
            setIsFlying(false);
            setGameStarted(false);
            setStatusMessage(`Cashed out: ${multiplier.toFixed(2)}x, You won ${winnings.toFixed(2)} credits!`);
            setMultiplier(1.00);
            setPositions([]);
        } else {
    ...
    const pathData = positions.map((pos, index) => `${index === 0 ? 'M' : 'L'} ${pos.x} ${400 - pos.y}`).join(' ');
                ...
                <div className="flex flex-row sm:flex-row gap-4 mb-4 items-center">
                    <button 
                        ...
                        className={`px-6 py-2 rounded-lg text-white font-semibold transition-transform duration-300 ${
                            isFlying ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:-translate-y-1 hover:bg-blue-800'
                        }`}
                    >
                        Place Bet
                    </button>
                    ...
                    <button 
                        ...
                        disabled={!isFlying}
                        className={`px-6 py-2 rounded-lg text-white font-semibold transition-transform duration-300 ${
                            !isFlying ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-600 hover:-translate-y-1 hover:bg-green-800'
                        }`}
                    >
                        Cash Out
                    </button>
